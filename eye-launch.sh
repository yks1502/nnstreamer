...
# Cropped only left eye with face-landmark
# gst-launch-1.0 -m \
#   tensor_videocrop name=crop_left \
#   v4l2src name=cam_src ! videoconvert ! videoscale ! \
#   video/x-raw,width=1000,height=1000,format=RGB,pixel-aspect-ratio=1/1,framerate=10/1 ! tee name=t \
...
#   crop_left.src ! tee name=eye_left \
#   eye_left. ! videoconvert ! videoscale ! video/x-raw,width=64,height=64,format=RGB,framerate=10/1 ! videoconvert ! videoscale ! mix_left.sink_1 \
#   eye_left. ! videoconvert ! videoscale ! video/x-raw,width=64,height=64,format=RGB,framerate=10/1 ! \
...
#   compositor name=mix_left sink_0::zorder=2 sink_1::zorder=1 ! videoconvert ! ximagesink sync=false \
...
gst-launch-1.0 -m \
  ...
  crop_left.src ! tee name=eye_left \
  crop_right.src ! tee name=eye_right \
  eye_left. ! videoconvert ! videoscale ! video/x-raw,width=64,height=64,format=RGB,framerate=10/1 ! videoconvert ! videoscale ! mix_left.sink_1 \
  eye_left. ! videoconvert ! videoscale ! video/x-raw,width=64,height=64,format=RGB,framerate=10/1 ! \
    tensor_converter ! tensor_transform mode=arithmetic option=typecast:float32,div:255.0 ! \
    queue ! tensor_filter framework=tensorflow2-lite model=iris_landmark.tflite custom=Delegate:XNNPACK,NumThreads:4 ! \
    ...
    videoconvert ! videoscale ! mix_left.sink_0 \
  compositor name=mix_left sink_0::zorder=2 sink_1::zorder=1 ! videoconvert ! ximagesink sync=false \
  eye_right. ! videoconvert ! videoscale ! video/x-raw,width=64,height=64,format=RGB,framerate=10/1 ! videoconvert ! videoscale !mix_right.sink_1 \
  eye_right. ! videoconvert ! videoscale ! video/x-raw,width=64,height=64,format=RGB,framerate=10/1 ! \
    tensor_converter ! tensor_transform mode=arithmetic option=typecast:float32,div:255.0 ! \
    queue ! tensor_filter framework=tensorflow2-lite model=iris_landmark.tflite custom=Delegate:XNNPACK,NumThreads:4 ! \
    other/tensors,num_tensors=2,types=\"float32\,float32\",dimensions=\"213:1:1:1\,15:1:1:1\",format=static ! \
    tensor_decoder mode=eye_detecting option1=64:64 ! \
    videoconvert ! videoscale ! mix_right.sink_0 \
  compositor name=mix_right sink_0::zorder=2 sink_1::zorder=1 ! videoconvert ! ximagesink sync=false \
